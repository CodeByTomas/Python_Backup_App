...
class BackupApp:
    ...
    def setup_ui(self):
        ...
        self.cancel_btn = tk.Button(ctrl_frame, text="Cancel Backup", command=self.cancel_backup_operation, state="disabled")
                ...
                for file in files:
                    ...
                    if os.path.isfile(file_path):
                        try:
                            ...
                            self.log_message(f"Error getting size for {file_path}: {e}", level="error")
        return total
        ...
        for src in self.source_folders:
            ...
            for root_dir, _, files in os.walk(src):
                ...
                try:
                    ...
                    self.log_message(f"Could not create directory {current_dest}: {e}", level="error")
                    continue
                for file in files:
                    if self.cancel_backup:
                        self.root.after(0, self.backup_canceled)
